...
exports.addToWishlist = async (req, res) => {
    try {
        ...
        return res.status(201).json({ message: 'Sản phẩm đã được thêm vào danh sách yêu thích', wishlist });
    } catch (error) {
...
exports.removeFromWishlist = async (req, res) => {
    try {
        ...
        if (!wishlist) {
            return res.status(404).json({ error: 'Danh sách yêu thích không tồn tại' });
        }

        wishlist.products = wishlist.products.filter(item => item.product.toString() !== productId);
        await wishlist.save();

        return res.status(200).json({ message: 'Sản phẩm đã được xóa khỏi danh sách yêu thích', wishlist });
    } catch (error) {
        handleError(res, error);
    }
};
